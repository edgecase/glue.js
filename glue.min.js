...
Glue.prototype.notify=function(a,b,c){function e(a,b,c,f,e){if(!_.isEqual(c,Glue.deepClone(f))){c={operation:a,oldValue:c,currentValue:f};if(!_.isUndefined(e))c.index=e;_.isEmpty(b.operations)?b.callback.call(b.context,c):_.include(b.operations,a)&&b.callback.call(b.context,c)}}var f=this;_.each(this.listeners.specific,function(b,c){var h=f.get(c),i=f.get(c,f.cache);_.each(b,function(b){e(a,b,i,h)})});(function(a,g,h,i){a=Glue.permutateKey(a);_.each(a,function(a){var b=a[0],c=a[2];_.each(f.listeners.generic[a[1]],
function(a){e(g,a,f.get(b,h),f.get(b,i),c)})});var l={};_.each(f.listeners.generic,function(a,c){c.match(RegExp("^"+b.replace("[","\\[").replace("]","\\]")))&&(l[c]=a)});_.each(l,function(a,b){var d=b.replace(/\[[^\[]*\]$/,""),j=f.get(d,h),k=f.get(d,i),d=j.length>k.length?j.length:k.length,d=c&&c.reverse?_.range(0,d).reverse():_.range(0,d);_.each(d,function(b){var c=j[b],d=k[b];_.each(a,function(a){e(g,a,c,d,b)})})})})(b,a,f.cache,f.target)};
