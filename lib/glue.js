  ...
  function add(key, listener, target) {
    if (key === Glue.ANY_KEYPATH) {
      ...
      if (key.match(/\#|.+\(\)$/)) {
        ...
        self.listeners.computed[key].push({ target: target, func: listener });
      } else {
...
Glue.prototype.removeListener = function() {
  ...
  if (arguments.length === 0) {
    ...
    if (arg === Glue.ANY_KEYPATH) {
      ...
      _.each(['assigned', 'computed'], function(listenerType) {
        _.each(self.listeners[listenerType], function(listeners, key) {
          self.listeners[listenerType][key] = _.reject(listeners, function(listener) {
            return listener.target === arg;
          });
        });
      });
