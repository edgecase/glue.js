...
Glue.prototype.notify = function(operation, key) {
  ...
  if (key.match(/\]$/)) invokeGeneric(key, operation, self.cache, self.target);
  ...
  function invokeGeneric(k, operation, oldTarget, currentTarget) {
    ...
    _.each(affectedChildren, function(listeners, k) {
      var base    = k.replace(/\[[^\[]*\]$/, ''),
          ...
          maxIndex = old.length > current.length ? old.length : current.length;

      _.each(_.range(0, maxIndex), function(index) {
