...
Glue.keyPointsToArray = function(key) {
...
Glue.prototype.notify = function(key, msg, opts) {
  ...
  opts = _.defaults({descend: true}, opts);

  if (!_.isEqual(msg.oldValue, msg.newValue)) {
    invoke(self.listeners.any, msg);
    ...
    _.each(self.listeners.computed, function(listeners, key) {
      ...
      if (newValue !== oldValue) {
        ...
        invoke(listeners, msg, 'computed', key);
      }
    });

    if (opts.descend) {
      decendingNotification(key.split('.'));
    } else {
      invoke(key);
    }
  ...
  function decendingNotification(keySegments) {
    if (!_.isEmpty(keySegments)) {
      var key = keySegments.join('.');

      invoke(self.listeners.assigned[key], msg, 'assigned', key);
      decendingNotification(_.initial(keySegments));
    }
  }

  function invoke(listeners, message, type, key) {
    _.each(listeners, function(listener) {
      listener.func.call(listener.target, message);
    });

    if (Glue.keyPointsToArray(key)) {
      var index = parseInt(key.match(/\[(\d*)]$/)[1])
        , arrKey = key.replace(/\d*(?=\]$)/, '');

        invoke(self.listeners[type][arrKey], _.extend(_.clone(message), { index: index}));
    }
  }
...
Glue.prototype.push = function() {
  ...
  function push(key, collection, item) {
    ...
    self.notify(key + '[' + collection.length + ']', {
        operation: "push"
      , newValue: item
    });
  };
...
Glue.prototype.pop = function(){
  ...
  _.each(keys.replace(/\s/g, '').split(','), function(key) {
    var collection = self.get(key);
    value = collection.pop();

    self.notify(key + '[' + collection.length+1 + ']', {
        operation: "pop"
      , oldValue: value
    });
  });
