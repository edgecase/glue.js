...
* Signature
*
* topic.filter(handler, [callback]);
* topic.filter('col', handler, [callback];
* topic.filter('v1.col', handler, [callback]);
* topic.filter('arr[i].col', handler, [callback]);
*
*/
Glue.prototype.filter = function() {
  ...
  function f(handler, key) {
    ...
    return key ? this.get(key) : this.target;
  }
};
