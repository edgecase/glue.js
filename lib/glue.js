...
Glue.prototype.addListener = function() {
  ...
  function add(keysAndOperations, callback, context) {
...
Glue.prototype.notify = function(operation, original, current, changes) {
  ...
  function callListener(listener, message) {
    if (message.oldValue !== message.currentValue || message.oldIndex !== message.currentIndex) {
...
Glue.prototype.push = function() {
  ...
  function push(key, collection, item) {
    var original = Glue.deepClone(self.target),
        current;

    collection.push(item);
    current = Glue.deepClone(self.target);

    self.notify("push", original, current, [
      { current: key + '[' + (self.get(key).length-1) + ']' }
    ]);
  };
...
Glue.prototype.pop = function(key){
  ...
  var collection = this.get(key),
      original = Glue.deepClone(this.target);

  var value = collection.pop(),
      current = Glue.deepClone(this.target);

  this.notify("pop", original, current, [{
    old: key + '[' + collection.length+1 + ']'
  }]);
...
Glue.prototype.insert = function() {
  ...
  var current = Glue.deepClone(this.target),
      changes = [{ current: key + '[' + index + ']'}]

  _.each(_.range(index+1, collection.length), function(i) {
    changes.push({ old: key + '[' + (i-1) + ']', current: key + '[' + i + ']'});
  });

  this.notify('insert', original, current, changes);
...
Glue.prototype.filter = function() {
  ...
  var key     = a.length < 2 ?  '' : a[i++],
      handler = a[i++];

  var collection = key === '' ? this.target : self.get(key),
      original = Glue.deepClone(this.target);

  remove = _.without(_.map(collection, function(value, index) {
    if (!handler(value)) return index;
  }), undefined).reverse();
  ...
  var current = Glue.deepClone(this.target),
      changes = _.map(remove, function(index) { return { old: key + '[' + index + ']' }; });

  this.notify("filter", original, current, changes);
};
...
Glue.prototype.set = function(key, value) {
  ...
  this.notify('set', original, current, [
    {old: key, current: key}
  ]);
...
Glue.prototype.remove = function(key){
  var original = Glue.deepClone(this.target)
      match    = key.match(/\[(\d+)\]$/);
  ...
  this.notify("remove", original, Glue.deepClone(this.target), [{ old: key }]);
