  ...
  function invokeGeneric(k, operation, oldTarget, currentTarget) {
    ...
    _.each(affectedChildren, function(listeners, k) {
      var base     = k.replace(/\[[^\[]*\]$/, ''),
          ...
          range = opts.reverse ? _.range(0, maxRange).reverse() : _.range(0, maxRange);
...
Glue.prototype.sortBy = function() {
  ...
  if (key === '') {
    this.target = _.map(sortedWithIndex, function(elem) { return elem[0] });
  } else {
    bs = this.baseKeyAndSuffix(key);
    bs[0][bs[1]] = _.map(sortedWithIndex, function(elem) { return elem[0] });
  };
