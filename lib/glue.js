...
Glue.prototype.removeListener = function() {
    ...
    if (key === '*' || key === void 0) {
      if (target) {
        ...
        _.each(self.listeners.computed, function(listeners, key) {
          self.listeners.computed[key] = removeListenerType('computed', key, target)

          if (self.listeners.computed[key].length === 0) {
            delete self.listeners.oldComputedValues[key];
          };
        });

        _.each(self.listeners.assigned, function(listeners, key) {
          self.listeners.assigned[key] = removeListenerType('assigned', key, target)
        });
      } else {
        self.resetListeners();
      }
  ...
  function removeListenerType(type, key, target) {
    return _.reject(self.listeners[type][key], function(listener) {
      return listener.target === target;
    });
  }
