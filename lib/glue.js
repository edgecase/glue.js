...
Glue.prototype.removeListener = function() {
  ...
  if (keyPath === this.anyKeyPath) {
    _.each(this.listeners, function(listener_group, keyPath) {
      var listeners = _.reject(listener_group, function(listener) {
        return _.isEqual(listener.hollaback, hollaback);
      });

      if(_.isEmpty(listeners)) {
        delete self.listeners[keyPath];
      } else {
        self.listeners[keyPath] = listeners;
      }
    });
  }
